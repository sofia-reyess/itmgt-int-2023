...
def shift_letter(letter, shift):
    ...
    *Note: the single underscore `_` is used to acknowledge the presence
    ...
    shift_letter = alphabet[(alphabet.index(final_letter) + shift) % len(alphabet)] 
...
def shift_by_letter(letter, letter_shift):
    ...
    alphabet = list("ABCDEFGHIJKLMNOPQRSTUVWXYZ")
    final_letter = letter.upper()
    shift_letter = alphabet[(alphabet.index(final_letter) + alphabet.index(letter_shift.upper())) % len(alphabet)]
...
def scytale_decipher(message, shift):
    ...
    # Replace `pass` with your code.
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    original_length = (len(message) // shift) * shift
